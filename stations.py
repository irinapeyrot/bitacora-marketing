...
# ---- 1. ESPINA por la banda inferior (tierra continua de izq a der) ----
#    Puntos de control elegidos a mano sobre tierra firme (ver mapa de relieve).
#    Avance monótono en X, ondulación suave => sin cruces, todo en tierra.
ctrl = [
    (1030, 1245), (1150, 1235), (1280, 1250), (1410, 1235),
    (1520, 1200), (1610, 1150), (1700, 1170), (1800, 1140),
    (1910, 1120), (2020, 1110), (2120, 1120),
]
def catmull(p, n=60):
    pts=[]
...
# ---- 3. repartir las 25 estaciones a lo largo de la espina, equiespaciadas por longitud ----
def arclen(pts):
    L=[0]
    for i in range(1,len(pts)):
        L.append(L[-1]+math.hypot(pts[i][0]-pts[i-1][0], pts[i][1]-pts[i-1][1]))
    return L
L=arclen(spine); total=L[-1]
pts=[]
for i in range(N):
    target=total*i/(N-1)
    j=0
    while j<len(L)-1 and L[j]<target: j+=1
    pts.append(spine[j])
...
def nearest_land_y(x, y, maxd=60):
