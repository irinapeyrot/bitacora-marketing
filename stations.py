...
random.seed(4)
pts = []
# zigzag dentro del rango REAL de tierra de la isla (x≈960..2180)
x0, x1 = 985, 2170
amp = 150
waves = 5.0
for i in range(N):
    t = i / (N - 1)
    x = x0 + t * (x1 - x0)
    y = cy + amp * math.sin(t * math.pi * waves)
    y += (t - 0.5) * 60
    x += random.uniform(-8, 8)
    y += random.uniform(-8, 8)
    x,y = push_to_land(x,y)       # respaldo: si igual cae en agua/lago, a tierra
    pts.append((x, y))

# separar pines demasiado cercanos (mínimo 55px) para que no se amontonen
def dist(a,b): return math.hypot(a[0]-b[0],a[1]-b[1])
for _ in range(3):
    for i in range(1,N):
        if dist(pts[i],pts[i-1])<62:
            dx=pts[i][0]-pts[i-1][0]; dy=pts[i][1]-pts[i-1][1]
            L=math.hypot(dx,dy) or 1
            nx=pts[i-1][0]+dx/L*68; ny=pts[i-1][1]+dy/L*68
            nx,ny=push_to_land(nx,ny)
            pts[i]=(nx,ny)
