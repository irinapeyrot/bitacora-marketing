...
(function () {
  ...
  function renderMatriz(fig, uid) {
    ...
    const ejeX = (fig.ejeX && fig.ejeX.length === 2)
      ? `<div class="mx-axis-x">
           <span><span class="mx-arw">←</span>${esc(fig.ejeX[0])}</span>
           <span>${esc(fig.ejeX[1])}<span class="mx-arw">→</span></span>
         </div>` : '';
    const ejeY = fig.ejeY
      ? `<div class="mx-axis">
           <span class="mx-arw-v">↑</span>
           <span class="mx-axis-lbl">${esc(fig.ejeY[0])}</span>
           <span class="mx-axis-lbl">${esc(fig.ejeY[1])}</span>
           <span class="mx-arw-v">↓</span>
         </div>` : '';
    return marco(fig, `
